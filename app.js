  ...
  const handleFileSelect = (e) => {
    if (e.target.files && e.target.files[0]) {
        ...
        if (!auth.currentUser) {
             ...
             updateProgress({ status: 'waiting_approval', uploadedAt: new Date().toISOString() });
             setStep('pending');
        }
    }
  };
...
const AdminPanel = ({ onClose, currentData }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-black flex items-center justify-center p-4">
            <div className="bg-gray-900 border border-green-500 p-6 rounded w-full max-w-sm font-mono text-green-500">
                ...
                <div className="mb-4 text-xs border border-green-900 p-2"> <p>User Stage: {currentData?.stage || 0}</p> <p>Status: {currentData?.status || 'N/A'}</p> </div>
